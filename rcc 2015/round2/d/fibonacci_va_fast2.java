 ...
 */
public class fibonacci_va_fast2 {
    public static void main(String[] args) {
        new fibonacci_va_fast2().run();
    }
    ...
    public void precalc() {
        ...
        pows = new int[d.length][45162][16];
        for (int i = 0; i < d.length; i++) {
            for (int j = 0; j < pows[i].length; j++) {
                pows[i][j][0] = j % d[i];
                for (int k = 1; k < pows[i][j].length; k++) {
                    pows[i][j][k] = (pows[i][j][k - 1] * pows[i][j][k - 1]) % d[i];
                }
            }
        }
    }
...
//        return b == 0 ? a : gcd(b, a % b);
    ...
    public int pow(int x, long p, int mod) {
        ...
        int ans = 1;
        for (int i = 0; i < pows[mod][x].length; i++) {
            if ((p & 1) == 1) {
                ans = (ans * pows[mod][x][i]) % d[mod];
            }
            p >>= 1;
        }
        return ans;
    ...
    public void solve() throws IOException {
        ...
        for (int i = 0; i < d.length; i++) {
            ...
            for (int j = 2; j < ps.length; j++) {
                ps[j] = (ps[j - 1] + pow(f1, gcd(f1, d[i]) != 0 ? k % phi[i] : k2, i));
                if (ps[j] >= d[i]) {
                    ps[j] -= d[i];
                }
                int f = (f1 + f2);
