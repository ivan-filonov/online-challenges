...
public class Validate {
    final int MAXN = 100;
    final int MAXSUMN = 10000;
    final int MAXX = 1000;

    public void run() {
        StrictScanner inf = new StrictScanner(System.in);
        int t = inf.nextInt();
        ensureLimits(t, 1, MAXSUMN, "Incorrect t");
        int sum = 0;
        for (int i = 0; i < t; i++) {
            inf.nextLine();
            int n = inf.nextInt();
            ...
            ensureLimits(l, 1, n, "Incorrect l in test #" + String.valueOf(i + 1));
            sum += n;
            ensureLimits(sum, 1, MAXSUMN, "Incorrect sum after test #" + String.valueOf(i + 1));
            inf.nextLine();
            for (int j = 0; j < n; j++) {
                int a = inf.nextInt();
                ensureLimits(a, 1, MAXX, String.format("Incorrect a[%d] = %d in test #%d", j + 1, a, i + 1));
            }
            inf.nextLine();
            for (int j = 0; j < n; j++) {
                int b = inf.nextInt();
                ensureLimits(b, 1, MAXX, String.format("Incorrect b[%d] = %d in test #%d", j + 1, b, i + 1));
            }
        }
        inf.nextLine();
        inf.close();
