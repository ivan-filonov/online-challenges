...
namespace {
  ...
  void process(std::string line) {
    ...
    std::string t;
    std::vector<char> package, addr1(4, 0), addr2(4, 0);

    std::string delim { "... " };
    
    for(int i = 0; i < 4; ++i) {
      std::getline(ss, t, delim[i]);
      addr1[i] = std::stoi(t);
    }
    ...
    while(std::getline(ss, t, ' ') && package.size() < 20) {
      package.push_back(std::stoi(t, nullptr, 16));
    }

    for(int i = 0; i < 4; ++i) {
      package[i + 12] = addr1[i];
      package[i + 16] = addr2[i];
    }

    uint16_t * svec = (uint16_t*)package.data();
    svec[5] = 0;
    uint32_t s32 = 0;
    for(int i = 0; i < 10; ++i) {
      s32 += svec[i];
    ...
    while(s32 >> 16) {
      s32 = (0xffff & s32) + (s32 >> 16);
    }
    svec[5] = 0xffff & ~s32;
    std::cout << std::hex;
    for(int i = 0; i < 20; ++i) {
      if(i)std::cout << ' ';
      std::cout << (0xF & (int)package[i]>>4);
      std::cout << (0xF & (int)package[i]);
    }
    std::cout << "\n";
  }
}
